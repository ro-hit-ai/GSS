...
function friendlyDbError(PDOException $e): string {
    ...
    $detail = $driverMsg !== '' ? (' (' . $driverMsg . ')') : '';
    return 'Database error. Please try again.' . $detail;
}
...
try {
    ...
    if ($userId > 0) {
        ...
        if ($sendEmail) {
            ...
            $subject = 'Your VATI GSS Login Details';
            $body = '<p>Hello ' . htmlspecialchars($firstName) . ',</p>' .
                '<p>Your account has been created. Use the following details to login:</p>' .
