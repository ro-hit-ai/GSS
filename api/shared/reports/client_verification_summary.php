...
try {
    ...
    // 2) steps: job_role_id, stage_key, verification_type_id, execution_group, assigned_role, is_active, type_name, type_category, required_count
    $spRoles = [];
    $spSteps = [];
    try {
        ...
        if (is_array($spSteps) && count($spSteps) > 0) {
            // Enrich required_count from Vati_Payfiller_Job_Role_Verification_Types if SP did not include it
            $needRc = false;
            foreach ($spSteps as $ss) {
                if (!isset($ss['required_count'])) { $needRc = true; break; }
            }
            if ($needRc) {
                try {
                    $rcStmt = $pdo->prepare(
                        'SELECT job_role_id, verification_type_id, required_count'
                        . ' FROM Vati_Payfiller_Job_Role_Verification_Types'
                        . ' WHERE job_role_id IN ('
                        .   'SELECT jr2.job_role_id FROM Vati_Payfiller_Job_Roles jr2 WHERE jr2.client_id = ?'
                        . ')'
                    );
                    $rcStmt->execute([$clientId]);
                    $rcRows = $rcStmt->fetchAll(PDO::FETCH_ASSOC);
                    $rcMap = [];
                    foreach ($rcRows as $rc) {
                        $k = (int)$rc['job_role_id'] . '_' . (int)$rc['verification_type_id'];
                        $rcMap[$k] = (int)$rc['required_count'];
                    }
                    foreach ($spSteps as $idx => $ss) {
                        $k = (int)($ss['job_role_id'] ?? 0) . '_' . (int)($ss['verification_type_id'] ?? 0);
                        $spSteps[$idx]['required_count'] = $rcMap[$k] ?? 1;
                    }
                } catch (Throwable $rcEx) {
                    foreach ($spSteps as $idx => $ss) {
                        if (!isset($spSteps[$idx]['required_count'])) {
                            $spSteps[$idx]['required_count'] = 1;
                        }
                    }
                }
            }
    ...
    if (empty($steps)) {
        ...
        try {
            $stmt2 = $pdo->prepare(
                ...
                . '       COALESCE(jv.required_count, 1) AS required_count\n'
                ...
                . '  LEFT JOIN Vati_Payfiller_Job_Role_Verification_Types jv\n'
                . '         ON jv.job_role_id = s.job_role_id AND jv.verification_type_id = s.verification_type_id\n'
                . ' WHERE jr.client_id = ?\n'
    ...
    foreach ($steps as $s) {
        ...
        $stepsByRole[$rid][] = [
            ...
            'required_count' => isset($s['required_count']) ? max(1, (int)$s['required_count']) : 1,
        ];
    }
